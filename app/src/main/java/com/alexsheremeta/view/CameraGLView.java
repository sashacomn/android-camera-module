...
import com.alexsheremeta.glutils.GLDrawer2D;
import com.alexsheremeta.tasks.OnImageTakenListener;
import com.alexsheremeta.tasks.PhotoProceedTask;
...
public final class CameraGLView extends GLSurfaceView {
    ...
    public static final int FLASH_ENABLED = 0;
    public static final int FLASH_DISABLED = 1;
    ...
    private static int currentCameraId = CAMERA_ID;
    private int currentFlashlightMode = FLASH_AUTO;
    ...
    public int switchFlashMode() {
        ...
        switch (currentFlashlightMode) {
            case FLASH_AUTO:
                currentFlashlightMode = FLASH_DISABLED;
                mCameraHandler.mThread.setFlashMode(Camera.Parameters.FLASH_MODE_OFF);
                break;
            case FLASH_DISABLED:
                currentFlashlightMode = FLASH_ENABLED;
                mCameraHandler.mThread.setFlashMode(Camera.Parameters.FLASH_MODE_ON);
                break;
            case FLASH_ENABLED:
                currentFlashlightMode = FLASH_AUTO;
                mCameraHandler.mThread.setFlashMode(Camera.Parameters.FLASH_MODE_AUTO);
                break;
        }
...
//********************************************************************************
    ...
    public synchronized void takePhoto(OnImageTakenListener onImageTakenListener) {
        if (mCameraHandler != null) {
            mCameraHandler.takePhoto(onImageTakenListener);
        }
    }
...
//			if (DEBUG) Log.i(TAG, "onSurfaceCreated:Gl extensions: " + extensions);
            ...
            GLES20.glClearColor(Color.red(intColor) / 255.0f, Color.green(intColor) / 255.0f, Color.blue(intColor) / 255.0f, 1.0f);
...
//				parent.requestRender();
    ...
    public static final class CameraHandler extends Handler {
        ...
        public void takePhoto(OnImageTakenListener onImageTakenListener) {
            sendMessage(obtainMessage(MSG_TAKE_PHOTO, 0, 0, onImageTakenListener));
        }
        ...
        public void handleMessage(final Message msg) {
            switch (msg.what) {
                ...
                case MSG_PREVIEW_STOP:
                    ...
                    break;
                case MSG_TAKE_PHOTO:
                    mThread.getPicture((OnImageTakenListener) msg.obj);
                    break;
                default:
    ...
    private static final class CameraThread extends Thread {
        ...
        private final void startPreview(final int width, final int height) {
            ...
            if ((parent != null) && (mCamera == null)) {
                ...
                try {
                    ...
                    previewWidth = closestSize.width;
                    previewHeight = closestSize.height;
                    // request closest picture size for an aspect ratio issue on Nexus7
                    final Camera.Size pictureSize = getOptimalPreviewSize(
                    ...
                    parent.post(new Runnable() {
                        @Override
                        public void run() {
                            parent.setVideoSize(closestSize.width, closestSize.height);
                            // parent.setVideoSize(parent.getVideoWidth(), parent.getVideoHeight());
                        }
                    });
                    final SurfaceTexture st = parent.getSurfaceTexture();
                    st.setDefaultBufferSize(previewSize.width, previewSize.height);
                    mCamera.setPreviewTexture(st);
                    Log.e("PREV_S", previewSize.width + " " + previewSize.height);
        ...
        public void getPicture(final OnImageTakenListener onImageTakenListener) {
        ...
        public static Camera.Size getOptimalPreviewSize(List<Camera.Size> sizes) {
            ...
            Camera.Size optimalSize = size1.width * size1.height > size2.width * size2.height ? size1 : size2;
            return optimalSize;
        }
        ...
        private static Camera.Size getClosestSupportedSize(List<Camera.Size> supportedSizes, final int requestedWidth, final int requestedHeight) {
            return (Camera.Size) Collections.min(supportedSizes, new Comparator<Camera.Size>() {
                ...
                private int diff(final Camera.Size size) {
                    return Math.abs(requestedWidth - size.height) + Math.abs(requestedHeight - size.width);
                }
